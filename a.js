...
const setFilterState = (key, values) => {
  ...
  for (const el of filterEls(key)) {
    el.classList.toggle('active', values.length === 0 || values.includes(el.dataset[key]))
  }
}
...
const refilter = () => {
  ...
  for (const el of filtered.children) {
    ...
    if (rangeLabel) {
      ...
      const damagePerShot2 = +el.dataset.damage2
      ...
      const delaySemi = +el.dataset.delaysemi
      const delayBurst = +el.dataset.delayburst
      const delayPreReload = +el.dataset.delayprereload
      const reloadSpeedMul = 1
      // const reloadSpeedMul = 1.15
      // const reloadSpeedMul = 1.35
      const reloadTimeMul = 1/reloadSpeedMul
      const reloadTime = +el.dataset.reload * reloadTimeMul
      const reloadTimeAi = +el.dataset.reloadai
      const totalReloadEmptyTime = +el.dataset.reloadempty * reloadTimeMul
      const reloadSingle = el.dataset.reloadsingle === 'true'
      const rpm = +el.dataset.rpm
      const rpm2 = +el.dataset.rpm2
      const isMelee = el.dataset.class === 'melee'

      const trf = isMelee ? 1 : Math.max(0, Math.min(1, (targetRange - range) / (rangeMax - range || 0.001)))
      const damage = damagePerShot * lerp(1, mulDamageMin, trf)
      const dim = damage * magazine
      const trfAi = Math.max(0, Math.min(1, (targetRange - rangeAi) / (rangeMaxAi - rangeAi || 0.001)))
      const damageAiEnemy = damagePerShotAiEnemy * lerp(1, mulDamageMinAi, trf)
      const damageAiHeister = damagePerShotAiHeister * lerp(1, mulDamageMinAi, trf)

      const isSemi = el.dataset.firemodes.includes('1')
      const burst = el.dataset.firemodes.includes('2') ? 2 : el.dataset.firemodes.includes('3') ? 3 : 0
      const isAuto = el.dataset.firemodes.includes('4')
      const fireTime = Math.min(
        isSemi ? delaySemi : Infinity,
        // TODO: not sure this is correct
        burst ? (60/rpm * burst + delayBurst) / burst : Infinity,
        isAuto || isMelee ? (60/rpm) : Infinity,
      )
      const fireTime2 = 60/rpm2
      const fireTimeAi = fireTime // TODO: AT override
      const dpsInst = Math.max(
        damage / fireTime,
        isMelee ? damagePerShot2 / fireTime2 : 0,
      )
      const dps = isMelee ? dpsInst : Math.max(
        magazine > 1 ? damage * (magazine - 1) / (fireTime * (magazine - 2) + Math.max(delayPreReload, fireTime) + reloadTime * (reloadSingle ? magazine - 1 : 1)) : 0,
        magazine > 0 ? damage * magazine / (fireTime * (magazine - 1) + Math.max(delayPreReload, fireTime) + totalReloadEmptyTime) : 0,
      )
      const dpsAiEnemy = damageAiEnemy * magazine / (fireTimeAi * magazine + reloadTimeAi) || 0
      const dpsAiHeister = damageAiHeister * magazine / (fireTimeAi * magazine + reloadTimeAi) || 0
      const dpsInstAiEnemy = damageAiEnemy / fireTimeAi
      const dpsInstAiHeister = damageAiHeister / fireTimeAi
      ...
      if (dimEl) {
        dimEl.dataset.v = isMelee ? 0 : dim
        dimEl.textContent = isMelee ? '' : dim.toFixed(0)
      }
      ...
      const fdc = Math.max(
        magazine > 0 ? fireTime * magazine / (fireTime * (magazine - 1) + Math.max(delayPreReload, fireTime) + totalReloadEmptyTime) || 0 : 0,
        magazine > 1 ? fireTime * (magazine - 1) / (fireTime * (magazine - 2) + Math.max(delayPreReload, fireTime) + reloadTime * (reloadSingle ? magazine - 1 : 1)) : 0,
      )
      const fdcEl = el.querySelector('.fdc')
      if (fdcEl) {
        fdcEl.textContent = (fdc * 100).toFixed(0) + '%'
        fdcEl.dataset.v = fdc
      }
    }
    if (show) {
...
const loadParams = () => {
  ...
  if (params.get('firemode')) {
    setFilterState('firemode', params.get('firemode').split(','))
  }
